...
public class MsgUtils {
    ...
    public static String bytes2HexString(byte[] buffer) {
        if (buffer != null && buffer.length > 0) {
            char[] hexChars = new char[buffer.length * 3 - 1];
            for (int i = 0; i < buffer.length; i++) {
                hexChars[i * 3] = hexArray[(buffer[i] & 0xf0) >>> 4];
                hexChars[i * 3 + 1] = hexArray[(buffer[i] & 0x0f)];
    ...
    public static byte[] hexString2bytes(String hexStr) {
        ...
        for (int i = 0; i < s.length; i++) {
            result[i] = (byte) Integer.parseInt(s[i], 16);
        }
        return result;
    }

    public static int bytes2Int(byte[] input) {
        return bytes2Int(input, 0);
    }

    public static int bytes2Int(byte[] input, int offset) {
        if (input != null && (input.length - offset) >= 4) {
    ...
    public static long bytes2Long(byte[] input, int offset) {
        if (input != null && (input.length - offset) >= 8) {
    ...
    public static int bcd2int(int input) {
    ...
    public static int getCrc32(byte[] buf, int len) {
        int v1,v3,i, a1 = 0;
        for (i = 0; i < len; i++) {
            v1 = (buf[i] & 0xff) ^ (i & 0xff);
            v3 = v1 ^ ((a1 >>> 24) & 0xff);
    ...
    public static int getMeCRC(byte[] buf, int len) {
        byte v3, v1;
        int i, a1 = 0;
        for (i = 0; i < len; i++) {
            v1 = buf[i];
            v3 = (byte)(v1 ^ ((a1 >>> 24) & 0xff));
            a1 = crc_table[v3] ^ (a1 << 8);
        }
        return  a1;
    }
